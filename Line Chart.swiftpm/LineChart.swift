...
struct LineChart: View {
	...
	init(data: [CGPoint], enablePositionIndicator: Bool = true, lineMarkWidth: Double = 3, pointMarkSize: Double = .zero, numberOfGridlines: Int = 4, axisBaseValue: Double? = .zero, dashedGridlines: Bool = true) {
		self.data = data
			...
			HStack(spacing: 4) {
				...
				VStack(alignment: .trailing, spacing: yLegendOffset) {
					ForEach(0...numberOfGridlines, id: \.self) { gridNumber in
						Text("$\(yLegendLabel(for: gridNumber), specifier: "%.0f")").offset(x: .zero, y: yLegendPosition(for: gridNumber, geometrySize: geometry.size))
				...
				.background {
					ForEach((0...numberOfGridlines), id: \.self) { gridNumber in
						gridlines(at: yLegendLabel(for: gridNumber), in: geometry.size)
							.trim(from: .zero, to: animationProgress)
							.stroke(.gray, style: dashedGridlines ? StrokeStyle(lineWidth: gridNumber == numberOfGridlines ? 2 : 0.6, lineCap: .square, dash: [2, gridNumber == numberOfGridlines ? .zero : 2]) : StrokeStyle(lineWidth: gridNumber == .zero ? 2 : 0.4, lineCap: .square)
							)
							.clipped()
					}
				}
...
private extension LineChart {
	func lineMark(in geometry: GeometryProxy, closedPath: Bool = false) -> some Shape {
		Path { path in
			for (index, value) in data.enumerated() {
				...
				if index == .zero { path.move(to: CGPoint(x: xPosition, y: yPosition)) }
			...
			if closedPath {
				...
				path.addLine(to: CGPoint(x: .zero, y: geometry.size.height))
				path.closeSubpath()
			}
		}
	}
	...
	func positionIndicator(in geometry: GeometryProxy) -> some View {
		...
		let boxOffset = max(.zero, min(geometry.size.width - boxHeight, dragGestureXLocation - boxHeight / 2))
		...
		.gesture(
			DragGesture(minimumDistance: .zero)
				.onChanged { value in
					...
					if((value.location.x < geometry.size.width) && (value.location.x > .zero)) {
	...
	func gridlines(at height: CGFloat, in geometrySize: CGSize) -> Path {
		Path { path in
			path.move(to: CGPoint(x: .zero, y: (height - minY) * stepHeight(geometrySize: geometrySize)))
	...
	func chartAxis(in geometrySize: CGSize) -> Path {
		Path { path in
			path.move(to: CGPoint(x: geometrySize.width, y: .zero))
...
private extension LineChart {
	...
	mutating func setupChart() {
		if !data.isEmpty {
			...
			maxY = yLabels.max() != .zero ? yLabels.max() ?? .zero : 1
			minX = xLabels.min() ?? .zero
			maxX = xLabels.max() ?? .zero
		...
		for index in 1..<data.count {
			...
			if ((xLabel <= upperBound) && (xLabel >= lowerBound)) {
				...
				DispatchQueue.main.async {
					if round(xLabel) == xLabels[.zero] {
						currentYLabel = yLabels[.zero]
